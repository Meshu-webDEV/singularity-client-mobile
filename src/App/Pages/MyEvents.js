...
// Context
import MyEventsContext from '../../Context/My-Events/MyEventsContext';

// Other
import { formatDate } from '../../lib/utils';
import { EVENT_STATUS, EXPLORE_LOAD_TYPES } from '../../lib/constants';
...
const MyEvents = () => {
  ...
  // Stats click filter
  const handleFilterByStats = async (type = -1) => {
    if (type === -1) return;
  ...
  useEffect(() => {
    if (!events.length) initialLoadExploreEvents();
  }, []);
  ...
  return (
    ...
    >
      <div className='relative flex-grow h-full justify-center items-center flex flex-col space-y-3.5 text-whites-light'>
        {/* Stats */}
        <div className='3xs:z-50 3xs:absolute 3xs:bottom-full 3xs:right-2 3xs:transform 3xs:-translate-y-1 self-end w-64 px-2 rounded-md flex items-center bg-dark-backgroundDark shadow-tinycard'>
          <div className='w-full py-2 flex justify-around items-center'>
            {isLoading && isEmpty(stats) ? (
              ...
              <FadeAnimation fadeIn={!isEmpty(stats)}>
                <div className='flex w-full justify-around items-center'>
                  <div
                    onClick={() => handleFilterByStats(EVENT_STATUS.ONGOING)}
                    className='ks items-center flex flex-col text-whites-lights'
                  >
                    <span className='text-xxs font-medium'>
                      {stats.ongoing}
                    </span>
                    <span className='text-3xs opacity-70'>Ongoing</span>
                  </div>
                  <div
                    onClick={() => handleFilterByStats(EVENT_STATUS.UPCOMING)}
                    className='items-center flex flex-col text-whites-light'
                  >
                    <span className='text-3xs font-medium'>
                      {stats.upcoming}
                    </span>
                    <span className='text-3xs opacity-70'>Upcoming</span>
                  </div>
                  <div
                    onClick={() => handleFilterByStats(EVENT_STATUS.COMPLETED)}
                    className='items-center flex flex-col text-whites-light'
                  >
                    <span className='text-3xs font-medium'>
                      {stats.completed}
                    </span>
                    <span className='text-3xs opacity-70'>Completed</span>
                  </div>
                  <div className='items-center flex flex-col text-whites-light'>
                    <span className='text-3xs font-medium font-sans'>
                      {stats.total}
                    </span>
                    <span className='text-3xs opacity-70'>Total</span>
                  </div>
                </div>
              </FadeAnimation>
            )}
